...
class ConcurrentArrayQueue: public ConcurrentQueue<T>
{
    ...
    static ConcurrentArrayQueue<T> *ArrayQueueFactory(int max_size, string mode);    // 构造函数
    ...
    // 5. private methods
    void set_lock(string lockmode);                 
    ConcurrentArrayQueue(int _max_size, string _mode); 
};

template<typename T>
ConcurrentArrayQueue<T> *ConcurrentArrayQueue<T>::ArrayQueueFactory(int _max_size, string _mode) 
{
    if(_max_size<0)
    return NULL;
    else
...
template<typename T>
ConcurrentArrayQueue<T>::ConcurrentArrayQueue(int _max_size, string _mode)
{
    max_size=_max_size;
    data=new T [max_size];
    front=0;   // 初始的队首元素下标为0
...
template<typename T>
void ConcurrentArrayQueue<T>::set_lock(string lockmode)
{
