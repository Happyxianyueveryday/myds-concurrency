...
{
	lock_guard<mutex> other_guard(other.mut);    // 拷贝构造函数被调用时当前对象的成员尚未初始化和使用，因此只需要锁定拷贝源的互斥锁 
    size=other.size;
...
{
	lock_guard<mutex> other_guard(other.mut);    //  移动构造函数被调用时当前对象的成员尚未初始化和使用，因此只需要锁定拷贝源的互斥锁 
	size=other.size;
    max_size=other.max_size;
    data=other.data;
    pos=other.pos;
...
{
	lock_guard<mutex> guard(mut);    // 整个入栈操作必须互斥进行，即使是判断条件部分也一样 
