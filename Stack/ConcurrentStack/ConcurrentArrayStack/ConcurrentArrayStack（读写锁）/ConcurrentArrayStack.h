...
class ConcurrentArrayStack: public ConcurrentStack<T>
{
    ...
    ConcurrentArrayStack(int _max_size, string mode);
...
template<typename T>
ConcurrentArrayStack<T>::ConcurrentArrayStack(int _max_size, string _mode)
{   
    size=0;
...
template<typename T>
void ConcurrentArrayStack<T>::set_lock(string lockmode)
{
