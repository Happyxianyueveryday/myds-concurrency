#ifndef CONCURRENTBLOCKINGSTACK_H
#define CONCURRENTBLOCKINGSTACK_H
...
class ConcurrentBlockingStack: public BlockingStack<T>   
{
	...
	condition_variable_any push_var;    // 入栈信号量 
	condition_variable_any pop_var;     // 出栈信号量 
	recursive_mutex mut;                // 可重入互斥锁  
};  
...
{
	lock_guard<recursive_mutex> length_lock(mut);
    return size;
}
...
{
	lock_guard<recursive_mutex> empty_lock(mut);
	if(!size)
	return true;
	else
	return false;
...
{
	lock_guard<recursive_mutex> full_lock(mut);
	if(size==max_size)
	return true;
	else
	return false;
