 ...
 * note 2: 读写公平法: 是指当某个线程正在执行时，到达了若干个读线程和写线程，在当前线程运行完毕后，始终优先运行先到达的线程
 * note 3: 读写公平法的实现核心：新建一个互斥量seq，用该互斥量控制read_lock和write_lock两个方法互斥执行，从而保证先到的线程始终可以优先运行 
 */
class fair_rwmutex
{
	public: 
