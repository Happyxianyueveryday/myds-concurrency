...
class wrmutex
{
	...
	private:
	// 4. domains  
	mutex read_count_mut;       // 负责读者计数器read_count修改的互斥锁 
	int read_count;             // 读者数量的计数器 
	mutex write_count_mut;      // 负责写者计数器write_count的计数器 
	int write_count;            // 写者数量的计数器 
	mutex seq;                  // 作为队列的互斥量，到达的读写线程都首先在该互斥量上阻塞（相当于排队） 
	mutex temp_seq;             // 缓冲队列，到达的读线程首先进入缓冲队列，每次只将一个读线程放入队列seq中 
	mutex write_mut;            // 负责写者间互斥写的互斥锁 
	mutex read_mut;             // 负责实现写者优先的互斥锁 
};
