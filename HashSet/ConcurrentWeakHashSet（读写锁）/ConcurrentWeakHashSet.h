#ifndef CONCURRENT_WEAK_HASHSET_H       
#define CONCURRENT_WEAK_HASHSET_H  
...
template<typename T>
class ConcurrentWeakHashSet:ConcurrentSet<T>       
{
	public:
	// 1. constructor
	ConcurrentWeakHashSet(string _mode, unsigned int _curr_level=16);
	ConcurrentWeakHashSet(const vector<T> &data, string _mode, unsigned int _curr_level=16);
...
template<typename T>
ConcurrentWeakHashSet<T>::ConcurrentWeakHashSet(string _mode, unsigned int _curr_level):capacity((_curr_level>0)?_curr_level:16)
{
...
template<typename T>
ConcurrentWeakHashSet<T>::ConcurrentWeakHashSet(const vector<T> &data, string _mode, unsigned int _curr_level):capacity((_curr_level>0)?_curr_level:16)
{
	if(_mode=="write_first"||_mode=="read_first"||_mode=="fair")
	mode=_mode;
	else
...
template<typename T>
vector<T> ConcurrentWeakHashSet<T>::values()
{
	vector<T> res;
	for(int i=0;i<capacity;i++)
	{
 ...
 */ 
template<typename T>
string ConcurrentWeakHashSet<T>::toString()
{
	string res;
	for(int i=0;i<capacity;i++)
	{
