...
class ConcurrentWeakHashSet:public ConcurrentSet<T>       
{
	...
	ConcurrentWeakHashSet(const vector<T> &data, unsigned int _curr_level=16);
...
template<typename T>
ConcurrentWeakHashSet<T>::ConcurrentWeakHashSet(const vector<T> &data, unsigned int _curr_level):capacity((_curr_level>0)?_curr_level:16)
{
	segments=new Segment<T>* [capacity];
	for(int i=0;i<capacity;i++)
	{
