...
class Segment 
{
    public:
    // 1. constructor
    Segment(string _mode);                                // 构造函数
    Segment(string _mode, const vector<T> &data);         // 从数组构造哈希表
 ...
 */
template<typename T>
Segment<T>::Segment(string _mode)
{
	size=0;    									// 初始化元素个数为0
 ...
 */
template<typename T>
Segment<T>::Segment(string _mode, const vector<T> &_data)
{
	size=0;    									// 初始化元素个数为0
...
template<typename T>
void Segment<T>::read_lock()
{
	if(mode=="read_first")
	rw_mut.read_lock();
	else if(mode=="write_first")
	wr_mut.read_lock();
	else
	fair_mut.read_lock();
}
...
void Segment<T>::read_unlock()
{
	...
	fair_mut.read_unlock();
} 

template<typename T>
void Segment<T>::write_lock()
{
	if(mode=="read_first")
	rw_mut.write_lock();
	else if(mode=="write_first")
