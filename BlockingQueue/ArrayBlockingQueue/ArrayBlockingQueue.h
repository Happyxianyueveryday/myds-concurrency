...
class ArrayBlockingQueue: public BlockingQueue<T>
{
    ...
    recursive_mutex mut;       // 用于同步控制和并发的互斥量 
    condition_variable_any push_var;    // 入队线程的条件变量 
    condition_variable_any pop_var;     // 出队线程的条件变量 
...
void ArrayBlockingQueue<T>::push(T val) 
{
    ...
    unique_lock<recursive_mutex> ulock(mut);
